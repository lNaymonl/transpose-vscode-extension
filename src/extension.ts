...
function transposeArroundDot(text: string): string {
...
function transposeArroundUppercase(text: string): string {
...
function getCharBeforeAndAfterCursor(
...
export function activate(context: vscode.ExtensionContext) {
	const transposeSelectionDisposable = vscode.commands.registerCommand(
		'extension.transposeSelection',
		async function () {
						...
						for (let i = 0; i < selections.length; ++i) {
							if (document.getText(selections[i]).length > 0) {
								...
								if (text.includes('.')) {
									editorBuild.replace(
										selections[i],
										transposeArroundDot(text)
									);
								} else {
									editorBuild.replace(
										selections[i],
										transposeArroundUppercase(text)
									);
								}
							} else {
								const cursorPos = selections[i].anchor;
								const textRange =
									document.getWordRangeAtPosition(cursorPos);
	...
	const transposeCharacter = vscode.commands.registerCommand(
		'extension.transposeCharacter',
		() => {
			const editor = vscode.window.activeTextEditor;
				...
				editor.edit((editBuilder) => {
					if (selections.length > 0) {
						for (let i = 0; i < selections.length; ++i) {
							...
							const [beforeRange, afterRange] =
								getCharBeforeAndAfterCursor(cursor);
